...
/**
 * Library of functions for anonforum outside of the core api
 */
 ...
 */
class anonforum_portfolio_caller extends portfolio_module_caller_base {
    ...
    function get_return_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/anonforum/discuss.php?d=' . $this->discussion->id;
    }
    /**
     * @global object
     * @return array
    ...
    function get_navigation() {
        ...
        $navlinks[] = array(
            ...
            'link' => $CFG->wwwroot . '/mod/anonforum/discuss.php?d=' . $this->discussion->id,
            'type' => 'title'
        );
    ...
    function prepare_package() {
        ...
        if ($this->attachment) { // simplest case first - single file attachment
            ...
            if ($writingleap) {
                ...
                $selection = new portfolio_format_leap2a_entry('anonforumdiscussion' . $this->discussionid,
                    get_string('discussion', 'anonforum') . ': ' . $this->discussion->name, 'selection');
     ...
     * that corresponds to a single anonforum post,
    ...
    private function prepare_post_leap2a(portfolio_format_leap2a_writer $leapwriter, $post, $posthtml) {
        $entry = new portfolio_format_leap2a_entry('anonforumpost' . $post->id,  $post->subject, 'resource', $posthtml);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id])) {
            $leapwriter->link_files($entry, $this->keyedfiles[$post->id], 'anonforumpost' . $post->id . 'attachment');
        }
    ...
    /**
     * this is a very cut down version of what is in anonforum_make_mail_post
     *
     * @global object
     * @param int $post
     * @return string
    ...
    private function prepare_post($post, $fileoutputextras=null) {
        ...
        $formattedtext = portfolio_rewrite_pluginfile_urls($formattedtext, $this->modcontext->id, 'mod_anonforum', 'post', $post->id, $format);
        ...
        if (empty($post->anonymouspost)) {
            $by->name = fullname($users[$post->userid], $viewfullnames);
        } else {
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id]) && count($this->keyedfiles[$post->id]) > 0) {
            ...
            $output .= '<br /><b>' .  get_string('attachments', 'anonforum') . '</b>:<br /><br />';
    ...
    function check_permissions() {
        ...
        if ($this->post) {
            return (has_capability('mod/anonforum:exportpost', $context)
                || ($this->post->userid == $this->user->id
                    && has_capability('mod/anonforum:exportownpost', $context)));
        }
        return has_capability('mod/anonforum:exportdiscussion', $context);
    }
    /**
     * @return string
     */
    public static function display_name() {
        return get_string('modulename', 'anonforum');
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class anonforum_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if (($child = $this->browser->get_file_info($this->context, 'mod_anonforum', $this->filearea, $record->itemid))
