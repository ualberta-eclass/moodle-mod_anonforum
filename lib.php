...
function forum_add_instance($forum, $mform = null) {
    ...
    if ($forum->type == 'single') {  // Create related discussion.
        ...
        $discussion->anonymouspost = empty($forum->anonymous) ? 0 : $forum->anonymous;
        $message = '';
...
function forum_update_instance($forum, $mform) {
    ...
    if ($forum->type == 'single') {  // Update related discussion and post.
        ...
        if (!empty($discussions)) {
            ...
            $discussion->mailnow         = false;
            $discussion->groupid         = -1;
            $discussion->anonymouspost   = $forum->anonymous;
        ...
        foreach ($users as $userto) {
            ...
            foreach ($posts as $pid => $post) {
                ...
                if (!empty($post->anonymouspost)) {
                    ...
                    $cloneduserfrom = clone($userfrom);
                    $cloneduserfrom->email = $CFG->noreplyaddress;
                    $cloneduserfrom->firstname = get_string('anonymoususer', 'forum');
                    $cloneduserfrom->lastname = '';
                    $eventdata->userfrom = $cloneduserfrom;
                }

                $smallmessagestrings = new stdClass();
                $smallmessagestrings->user = empty($post->anonymouspost) ? fullname($userfrom) : get_string('anonymoususer', 'forum');
                    ...
                    foreach ($postsarray as $postid) {
                        ...
                        if ($maildigest == 2) {
                            ...
                            if (empty($post->anonymouspost)) {
                                ...
                                $by->name = get_string('anonymoususer', 'forum');
                            }
...
function forum_get_all_discussion_posts($discussionid, $sort, $tracking=false) {
    ...
    foreach ($posts as $pid=>$p) {
        ...
        if (!empty($p->anonymouspost)) {
            $p->firstname = get_string('anonymoususer', 'forum');
            $p->lastname = '';
            $p->email = '';
        }
    }
...
function forum_print_post($post, $discussion, $forum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    if (empty($post->anonymouspost)) {
        ...
        $postuser->firstname = get_string('anonymoususer', 'forum');
        $postuser->lastname  = '';
        $postuser->imagealt  = '';
        $postuser->picture   = '';
        $postuser->email     = '';
        ...
        $postuser->fullname    = get_string('anonymoususer', 'forum');
    ...
    // Output ratings
    if (!empty($post->rating) && empty($forum->anonymous)) {
...
function forum_print_discussion_header(&$post, $forum, $group=-1, $datestring="",
    ...
    if (empty($post->anonymouspost)) {
        ...
        echo get_string('anonymoususer', 'forum');
        echo "</td>\n";
    }
    ...
    $usermodified->firstname = get_string('anonymoususer', 'forum');
    $usermodified->lastname  = '';
    $usermodified->link = get_string('anonymoususer', 'forum').'<br />';

    // Check if we have a lastpost (or if this is the last post) and if it is not anonymous


    if (!empty($post->lastpostid)) {
        $lastpost = $DB->get_record('forum_posts', array( 'id' => $post->lastpostid), 'id, anonymouspost', 'anonymouspost');
        if (empty($lastpost->anonymouspost)) {
            $usermodified->id = $post->usermodified;
            $usermodified = username_load_fields_from_object($usermodified, $post, 'um');
            $usermodified->link = '<a href="'.$CFG->wwwroot.'/user/view.php?id='.$post->usermodified.'&amp;course='.$forum->course.'">'.
...
function forum_post_subscription($post, $forum) {
    ...
    if (empty($post->anonymouspost)) {
        ...
        $info->name  = get_string('anonymoususer', 'forum');
    }
